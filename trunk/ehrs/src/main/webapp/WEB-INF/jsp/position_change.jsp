...
<head>
	...
	<title>Main menu: change position</title>
...
<div id="page">
	<div id="content">
		<div class="post">
			<h2 class="title"><fmt:message key="general.change">Change</fmt:message> ${position.name}</h2>
			<div class="entry">
				...
				<form name="position" method="POST">
					...
					<c:if test="${result == '1'}">
						<p class="confirm"><fmt:message key="position.change.sucess">position information changed successfully.</fmt:message></p>
						<p><span style="color: red;"><fmt:message key="position.change.return">Return to this position? </fmt:message></span>
					...
					<c:if test="${result != 1}">			
					<fieldset>
					<legend style="font-weight: bold;"><fmt:message key="position.info">position information</fmt:message></legend>
					<table>
						<tr>
							<td><fmt:message key="position.name">Name*:</fmt:message></td>
							<td><input type="text" name='<c:out value="name" />' value='<c:out value="${position.name}" />' /></td>
						</tr>
						<tr>
							<td><fmt:message key="position.comments">Comments:</fmt:message></td>
							<td>
					...
					<input type="hidden" name='<c:out value="id" />' value='<c:out value="${position.id}" />' />
